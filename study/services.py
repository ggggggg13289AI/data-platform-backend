...
class StudyService:
    ...
    def get_studies_queryset(
        ...
        if exam_ids and len(exam_ids) > 0:
            # EXAM IDS FILTER: Filter by specific exam IDs (e.g. for export selected)
            placeholders = ','.join(['%s'] * len(exam_ids))
            conditions.append(f"exam_id IN ({placeholders})")
        ...
        # Default: order_datetime_desc (Most recent first - expected UX)
