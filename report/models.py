...
class Report(models.Model):
    ...
    content_hash = models.CharField(
        max_length=64,
        db_index=True,
        help_text='內容 SHA256 雜湊值，用於快速去重'
    )
    """內容 SHA256 雜湊，用於去重"""
    
    version_number = models.IntegerField(
        default=1,
        help_text='版本號，每次更新內容時遞增'
    )
    """版本號，從 1 開始"""
    ...
    created_at = models.DateTimeField(
        auto_now_add=True,
        db_index=True,
        help_text='記錄創建時間戳，自動設定為當前時間'
    )
    """創建時間"""
    
    updated_at = models.DateTimeField(
        auto_now=True,
        help_text='記錄更新時間戳，每次修改自動更新'
    )
    """更新時間"""
    ...
    metadata = models.JSONField(
        default=dict,
        blank=True,
        help_text='動態元資料，儲存額外的特定領域資訊 (JSON 格式)'
    )
    """動態元資料 (JSON)"""
...
class ReportVersion(models.Model):
    ...
    content_hash = models.CharField(
        max_length=64,
        db_index=True,
        help_text='內容 SHA256 雜湊值快照，追蹤內容變更'
    )
    """內容雜湊快照"""
    
    content_raw = models.TextField(
        help_text='報告內容的完整快照，保存版本發佈時的原始內容'
    )
    """內容快照"""
...
class ReportSummary(models.Model):
    ...
    short_summary = models.CharField(
        max_length=500,
        help_text='簡短摘要，約 100-200 字'
    )
    """簡短摘要 (~100-200 字)"""
    
    long_summary = models.TextField(
        help_text='詳細摘要，約 500-1000 字'
    )
    """詳細摘要 (~500-1000 字)"""
    ...
    key_points = models.JSONField(
        default=list,
        help_text='關鍵要點列表，JSON 數組格式，如 ["要點1", "要點2", ...]'
    )
    """關鍵要點列表 (JSON 數組)"""
...
class ReportSearchIndex(models.Model):
    ...
    search_text = models.TextField(
        db_index=True,
        help_text='可搜尋的文本，包含標題 + 處理過的內容 + 元資料'
    )
    """搜尋文本 (已索引)"""
    ...
    relevance_score = models.FloatField(
        default=1.0,
        help_text='相關性得分，用於搜尋結果排序，預設值為 1.0'
    )
    """相關性得分"""
...
class ExportTask(models.Model):
    ...
    query_params = models.JSONField(
        help_text='搜尋參數 (page_size, filters, sort 等)'
    )
    """查詢參數 (JSON)"""
    ...
    started_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='處理開始時間，當狀態變更為 processing 時設置'
    )
    """開始時間"""
...
class AIAnnotation(models.Model):
    ...
    content = models.TextField(
        help_text='註解內容，可為 JSON 字符串或純文本'
    )
    """註解內容 (JSON 或文本)"""
    ...
    created_at = models.DateTimeField(
        auto_now_add=True,
        db_index=True,
        help_text='註解建立時間，自動設定為當前時間'
    )
    """建立時間"""
    
    updated_at = models.DateTimeField(
        auto_now=True,
        help_text='註解更新時間，每次修改自動更新'
    )
    """更新時間"""
