...
class Command(BaseCommand):
    ...
    def _display_results(self, stats):
        ...
        self.stdout.write(
            f"  Records created: {self.style.SUCCESS(str(stats['created']))}{' records' if stats['created'] > 0 else ''}"
        )
        self.stdout.write(
            f"  Records updated: {self.style.WARNING(str(stats['updated']))}{' records' if stats['updated'] > 0 else ''}"
        )
        self.stdout.write(
            f"  Records deduplicated: {self.style.WARNING(str(stats['duplicated']))}{' records' if stats['duplicated'] > 0 else ''}"
        )
        self.stdout.write(
            f"  Records skipped: {self.style.HTTP_INFO(str(stats['skipped']))}{' records' if stats['skipped'] > 0 else ''}"
        )
        self.stdout.write(
            f"  Records with errors: {self.style.ERROR(str(stats['errors']))}{' records' if stats['errors'] > 0 else ''}"
        )
        ...
        if stats["errors"] == 0:
            ...
            self.stdout.write(
                self.style.WARNING(f"⚠️  Migration completed with {stats['errors']} errors")
            )
