...
class DateTimeHelper:
    ...
    def create_datetime_sequence(
        ...
        """
        return [
            base_date + timedelta(hours=i * interval_hours)
            for i in range(count)
        ]
...
class MockDataGenerator:
    ...
    EXAM_EQUIPMENTS = [
        'CT-SCANNER-01', 'CT-SCANNER-02',
        'MRI-MACHINE-01', 'MRI-MACHINE-02',
        'X-RAY-01', 'X-RAY-02'
    ]
    EXAM_DESCRIPTIONS = [
        'Chest CT', 'Brain MRI', 'Chest X-ray',
        'Abdominal CT', 'Spine MRI', 'Knee X-ray'
    ]
    EXAM_ROOMS = [
        'CT-ROOM-1', 'CT-ROOM-2',
        'MRI-ROOM-1', 'MRI-ROOM-2',
    ...
    def study_with_age(cls, age: int, exam_id: str = "AGE001") -> Dict[str, Any]:
        ...
        return StudyFactory.create_complete_study(
            exam_id=exam_id,
            patient_age=age,
            patient_birth_date=f'{birth_year}-01-01'
        )
...
class EdgeCaseGenerator:
    ...
    def special_character_names() -> List[Dict[str, Any]]:
        ...
        return [
            StudyFactory.create_complete_study(
                exam_id="SPEC001",
                patient_name="O'Brien, Patrick"
            ),
            StudyFactory.create_complete_study(
                exam_id="SPEC002",
                patient_name="García, José"
            ),
            StudyFactory.create_complete_study(
                exam_id="SPEC003",
                patient_name="李明 (Li Ming)"
            ),
        ]
