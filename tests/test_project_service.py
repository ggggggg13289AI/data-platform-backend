...
class ProjectServiceTestCase(TestCase):
    ...
    def test_get_project_statistics_returns_expected_fields(self):
        ...
        ProjectService.add_member(
            project=project, user_id=self.viewer.id, role=ProjectMember.ROLE_VIEWER
        )
...
class ProjectPermissionsTestCase(TestCase):
    ...
    def test_get_user_role_and_permissions(self):
        ...
        self.assertNotIn(
            ProjectPermissions.PERMISSION_DELETE,
            ProjectPermissions.get_user_permissions(self.project, self.viewer),
        )

    def test_can_manage_member_logic(self):
        can_owner_manage_admin = ProjectPermissions.can_manage_member(
            self.project, self.owner, self.admin
        )
        can_admin_manage_owner = ProjectPermissions.can_manage_member(
            self.project, self.admin, self.owner
        )
        can_admin_manage_viewer = ProjectPermissions.can_manage_member(
            self.project, self.admin, self.viewer
        )
